...
exports.BarAggregatedMV = new moose_lib_1.MaterializedView({
    ...
    selectStatement: (0, moose_lib_1.sql)(templateObject_1 || (templateObject_1 = __makeTemplateObject(["SELECT\n    toDayOfMonth(", ") as dayOfMonth,\n    count(", ") as totalRows,\n    countIf(", ") as rowsWithText,\n    sum(", ") as totalTextLength,\n    max(", ") as maxTextLength\n  FROM ", "\n  GROUP BY toDayOfMonth(utcTimestamp)\n  "], ["SELECT\n    toDayOfMonth(", ") as dayOfMonth,\n    count(", ") as totalRows,\n    countIf(", ") as rowsWithText,\n    sum(", ") as totalTextLength,\n    max(", ") as maxTextLength\n  FROM ", "\n  GROUP BY toDayOfMonth(utcTimestamp)\n  "])), barColumns.utcTimestamp, barColumns.primaryKey, barColumns.hasText, barColumns.textLength, barColumns.textLength, barTable),
